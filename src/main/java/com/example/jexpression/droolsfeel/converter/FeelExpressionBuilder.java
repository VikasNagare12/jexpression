...
public final class FeelExpressionBuilder {
    ...
    public static String toFeel(Validation v) {
        return switch (v.op()) {
            ...
            case "GreaterOrEqual" -> "%s >= %s".formatted(v.field(), value(v, 0));
            case "LessOrEqual" -> "%s <= %s".formatted(v.field(), value(v, 0));
            case "Greater" -> "%s > %s".formatted(v.field(), value(v, 0));
            case "Less" -> "%s < %s".formatted(v.field(), value(v, 0));
            case "Equals" -> "%s = %s".formatted(v.field(), value(v, 0));
            case "NotEquals" -> "%s != %s".formatted(v.field(), value(v, 0));
    ...
    private static String between(Validation v) {
        ...
        var from = value(v, 0);
        var to = value(v, 1);
        return "%s >= %s and %s <= %s".formatted(field, from, field, to);
    }
